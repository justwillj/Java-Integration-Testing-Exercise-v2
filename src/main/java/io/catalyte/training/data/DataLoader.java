...
public class DataLoader implements CommandLineRunner {
  ...
  private void loadVaccinations() {
    VaccinationRepository.save(
        new Vaccination(
            "Rabies",
            Date.from(LocalDate.parse("2019-10-17").atStartOfDay(ZoneId.systemDefault()).toInstant()),
            Pet1));
    VaccinationRepository.save(
        new Vaccination(
            "Worms",
            Date.from(LocalDate.parse("2020-04-12").atStartOfDay(ZoneId.systemDefault()).toInstant()),
            Pet1));
    VaccinationRepository.save(
        new Vaccination(
            "RVHD",
            Date.from(LocalDate.parse("2019-05-10").atStartOfDay(ZoneId.systemDefault()).toInstant()),
            Pet2));
    VaccinationRepository.save(
        new Vaccination(
            "RVHD",
            Date.from(LocalDate.parse("2019-06-08").atStartOfDay(ZoneId.systemDefault()).toInstant()),
            Pet3));
  }
}
